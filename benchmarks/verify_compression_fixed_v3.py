#!/usr/bin/env python3
"""
任务 1 & 2: 修复版本 v3 - 完全绕过模型加载
直接在随机 KV cache 上测试压缩算法质量
"""
...
def test_compression_quality(model_name, head_dim=128, seq_len=512, compression_ratio=2.0):
    """
    在随机 KV 上测试压缩质量（不加载模型）

    Args:
        model_name: 模型名称（仅用于报告）
        head_dim: Head dimension（默认 128，标准 Qwen 配置）
        seq_len: 序列长度
...
def main():
    ...
    # 任务 1: 测试压缩方法（使用已知 head_dim）
    print("\n" + "="*70)
    print("任务 1: 压缩方法质量测试")
    print("="*70)

    models = [
        ("/Volumes/toshiba/models/qwen3.5-0.8b-opus-distilled", "Qwen3.5-0.8B (混合架构)", 64),
        ("/Volumes/toshiba/models/qwen3-8b-mlx", "Qwen3-8B (纯 Transformer)", 128)
    ]

    for model_path, model_desc, head_dim in models:
        print(f"\n{'='*70}")
        print(f"测试: {model_desc}")
        print(f"{'='*70}")

        result = test_compression_quality(
            model_path,
            head_dim=head_dim,
            seq_len=512,
